...
<center>
  <div >
  <h1>Daftar Persediaan Barang Habis</h1>
  <h4>Klik buka stok untuk menambah stok ecer</h4><br>
  <?php 
  ...
  ?>
    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-6">
            <a href="buka_stok.php"> <button type="button" class="btn btn-primary">All</button></a>
        ...
        <div class="col-sm-6">
          <form class="navbar-form navbar-right" role="search" action="buka_stok.php" method="GET">
            <div class="form-group">
              <input type="text" class="form-control" name="caribarang" action="buka_stok.php" method="GET" required="required" placeholder="Nama barang . . .">
            </div>
...
<div style="background-color: white; width: 100%; border-radius: 10px; padding: 2%;">
  ...
  if(isset($_GET['caribarang'])){
    ...
    echo "
          ...
          $no = 0;
              $tampil=mysqli_query($connect, "SELECT * FROM vbarang WHERE namabarang LIKE '%$caribarang%' AND stok<=0"); 
              while ($data=mysqli_fetch_array($tampil)){
              $no++;
        ...
        echo "
            <tr>
              ...
              <td>$data[stok]</td>
              <td>
                <a href=tambah_buka_stok.php?id=$data[kodebarang]> <button class='btn btn-md btn-info'><svg-icon>Buka Stok</button></a>
              </td>
            </tr>";
              }
        echo "</table>";
...
<?php
  ...
  if(isset($_GET['kategori'])){
    ...
    echo "
          ...
          ";
              $no = 0;
              $tampil=mysqli_query($connect, "SELECT * FROM vbarang WHERE kodekategori = '$kategori' AND stok<=0 "); 
              while ($data=mysqli_fetch_array($tampil)){
              $no++;
              $hargajual=number_format($data['hargajual'],2,",",".");
  ...
  echo "
          ...
          $no = 0;
              $tampil=mysqli_query($connect, "SELECT * FROM vbarang WHERE stok<=0"); 
              while ($data=mysqli_fetch_array($tampil)){
              $no++;
              $hargajual=number_format($data['hargajual'],2,",",".");
