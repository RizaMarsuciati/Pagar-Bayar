...
<center>
  ...
  <h4>Informasi barang habis</h4><br>
  <?php 
...
<div style="background-color: white; width: 100%; border-radius: 10px; padding: 2%;">
  ...
  if(isset($_GET['caribarang'])){
    ...
    echo "
          ...
          $no = 0;
              $tampil=mysqli_query($connect, "SELECT * FROM vbarang WHERE namabarang LIKE '%$caribarang%' AND stokutuh<=0 OR stok<=0"); 
              while ($data=mysqli_fetch_array($tampil)){
              $no++;
...
<?php
  ...
  if(isset($_GET['kategori'])){
    ...
    echo "
          ...
          ";
              $no = 0;
              $tampil=mysqli_query($connect, "SELECT * FROM vbarang WHERE kodekategori = '$kategori' AND stokutuh<=0 OR stok<=0"); 
              while ($data=mysqli_fetch_array($tampil)){
              $no++;
              $hargabeli=number_format($data['hargabeli'],2,",",".");
              $hargajual=number_format($data['hargajual'],2,",",".");
  ...
  echo "
          ...
          $no = 0;
              $tampil=mysqli_query($connect, "SELECT * FROM vbarang where stokutuh<=0 OR stok<=0"); 
              while ($data=mysqli_fetch_array($tampil)){
              $no++;
              $hargabeli=number_format($data['hargabeli'],2,",",".");
              $hargajual=number_format($data['hargajual'],2,",",".");
